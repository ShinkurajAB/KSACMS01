...
namespace SwitchCMS.Component.Pages.Dashboard.Employee
{
    public partial class EmployeeResignation
    {
        ...
        EmployeeResignationPagination Pagination = new EmployeeResignationPagination();
        AuthenticationResponseModel response = new AuthenticationResponseModel();
        OUSR loginUserDetails = new OUSR();
        private int EditedResignationID = 0;
        private int DeletedResignationID = 0;
        ...
        protected async override Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender)
            {
                ...
                Pagination = await resignationService!.GetResignationsByCompanyId(Pagination, response.JwtToken!);
        ...
        {
            EditedResignationID = 0;
            await JSRuntime!.InvokeVoidAsync("CreateEmployeeResignationModal");
        }
        #endregion
        ...
        {
            EditedResignationID = HandoverID;
            await JSRuntime!.InvokeVoidAsync("CreateEmployeeResignationModal");
        ...
        {
            DeletedResignationID = ID;
            DeleteConfirmModalMessage = "Do you want to Delete this Resignation";
            this.StateHasChanged();
        ...
        private async Task DeleteConfirmed(bool IsDeleete)
        {
            if (IsDeleete)
            {
                var success = await resignationService!.DeleteResignation(DeletedResignationID, response.JwtToken!);
                if (success)
                {
                    Pagination = await resignationService!.GetResignationsByCompanyId(Pagination, response.JwtToken!);
                    this.StateHasChanged();
        ...
        #endregion
        #region Print EmployeeResignation
        private async Task PrintResignation(int ID)
        {
            string navigate = Utility.RouteData.PrintEmployeeResignation + "?ID=" + ID + "&token=" + response.JwtToken!;
